...
def single_round(state_array, payoff_dict, game_matrix, edge_list, control_nodes_list):
    for node in control_nodes_list:
        state_array[node] = rand_pick(0.5)
...
def process(b, edge_list, adj_dict):
    ...
    for _ in range(net_rep):
        ...
        ## centralized
        ## 5% --> 45
        # control_nodes_list = set([(i, j) for i in range(12, 19) for j in range(12, 19)]) \
        #                      - set([(12, 12), (12, 18), (18, 12), (18, 18)])
        ## 10% --> 90
        # control_nodes_list = set([(i, j) for i in range(11, 20) for j in range(10, 20)])
