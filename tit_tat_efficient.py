...
def single_round(payoff_array, game_matrix, edge_category, state_array, control_state, pre_state):
    ...
    for edge in control1_edge_sort:
        ...
        control_state[(nodex, nodey)] = pre_state[nodey]
    ...
    for edge in control2_edge:
        # both driver node
        nodex, nodey = edge
        control_state[(nodex, nodey)] = pre_control_state[(nodey, nodex)]
        control_state[(nodey, nodex)] = pre_control_state[(nodex, nodey)]
...
if __name__ == "__main__":
    ...
    file = "./fullgene_b_1_2_sf_k6_decentralized_controlrate_5_tit_tat.pk"
    if not os.path.exists(file):
        os.mknod(file)
    with open('./fullgene_b_1_2_sf_k6_decentralized_controlrate_5_tit_tat.pk', 'wb') as f:
